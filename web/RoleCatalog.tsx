...
export function RoleCatalog({
  ...
  const contasPool = poolDoCockpitAtivo
    ...
    : [];
  const contasElegiveis = contasPool
    .filter((account) => account.status !== "cooldown" && account.authenticated !== false)
    .slice()
    .sort((a, b) => Number(a.status === "ocupada") - Number(b.status === "ocupada"));
